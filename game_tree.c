...
void generate_levelBF(game_tree t, queue q)
{
	...
	for (int i = 0; i < MOVE_COUNT; i++) {
		...
		if (i == 0) {
			row = get_current_row(n) + VERT_MOVES[0];
			col = get_current_column(n) + VERT_MOVES[1];
		}
		else if (i == 1) {
			row = get_current_row(n) + VERT_MOVES[2];
			col = get_current_column(n) + VERT_MOVES[3];
		}
		else if (i == 2) {
			row = get_current_row(n) + HORIZ_MOVES[0];
			col = get_current_column(n) + HORIZ_MOVES[1];
		}
		else if (i == 3) {
			row = get_current_row(n) + HORIZ_MOVES[2];
			col = get_current_column(n) + HORIZ_MOVES[3];
		}
...
game_tree build_gameBF(game_tree t, queue q)
{
	...
	while (!is_empty_queue(q)) {
		c = front(q);
		game_state s = (game_state)(get_data(c));

		if (solved(s)) return c;
